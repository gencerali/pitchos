...
export async function writeArticles(articles, site, env) {
  ...
  for (let i = 0; i < articles.length; i++) {
    ...
    if (mode === 'template_official') {
      ...
      // Extract facts for story matching — all P4 articles, one Haiku call on full body.
      // Phase 1: replaced old 2-call classify+extract(blurb) with 1-call extractAndScore(body).
      if (article.is_p4) {
        try {
          const facts = await extractAndScore(article.full_text || '', article, env);
          if (!SKIP_STORY_TYPES.has(facts.story_type)) {
            published._facts = facts;
            console.log(`FACTS [${facts.story_type}] nvs=${facts.nvs_score}: "${article.title?.slice(0, 50)}"`);
          } else {
